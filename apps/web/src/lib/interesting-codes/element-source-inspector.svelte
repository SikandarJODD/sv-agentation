<script lang="ts">
	...
	const buildHoverInfo = (
		...
		const vscodeUrl =
			filePath === NO_SOURCE_VALUE ? null : buildVsCodeUrl(filePath, lineNumber, columnNumber);
	...
	>
		<div class="hover-header">
			<div class="hover-text">
				...
				<span>
					{hoverInfo?.shortFileName ?? NO_SOURCE_VALUE}:{hoverInfo?.lineNumber ??
						'?'}:{hoverInfo?.columnNumber ?? '?'}
				</span>
			</div>
