...
<section class="page-shell">
	<div class="hero">
		...
		<p class="hero-copy">
			The library works by taking a real DOM element from `bind:this` or `event.target`, then
			resolving its file path, line, column, component name, and stack. This playground keeps that
			logic inside a class-based rune controller so the examples stay tidy.
		</p>
		<p class="hero-note">
			Use this as a development-time debugging aid. It depends on Svelte source metadata and should
			not be treated as production-guaranteed behavior.
		</p>
	</div>
	...
	<div class="playground-grid">
		<div class="playground" bind:this={playground_element}>
			...
			<section class="demo-card" data-inspector-ui>
				...
				<p class="micro-copy">
					When inspector mode is on, click anything inside this playground except the control
					buttons.
				</p>
		...
		<aside class="result-panel" data-inspector-ui>
			...
			{:else}
				<p class="status">
					Inspect a bound element, toggle click inspector mode, or use the nested card.
				</p>
			{/if}
		</aside>
	</div>
